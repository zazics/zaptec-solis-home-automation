...
export class DailyAggregationService {
  ...
  public async calculateAndStoreDailyAggregation(date: Date): Promise<DailyAggregation> {
    ...
    try {
      ...
      this.logger.log(`Successfully calculated aggregation for ${dateStr}: ${aggregation.solarProduction.totalEnergyKwh.toFixed(2)} kWh solar`);
      return aggregation;
  ...
  private calculateGridAggregation(solisData: any[]) {
    ...
    const importedEnergyKwh = this.calculateTotalEnergy(
      solisData, 
      (item) => item.grid?.activePower > 0 ? item.grid.activePower : 0
    );

    const exportedEnergyKwh = this.calculateTotalEnergy(
      solisData, 
      (item) => item.grid?.activePower < 0 ? Math.abs(item.grid.activePower) : 0
    );
  ...
  private calculateZaptecAggregation(zaptecData: any[]) {
    ...
    const totalEnergyKwh = this.calculateTotalEnergy(
      zaptecData, 
      (item) => item.charging ? item.power || 0 : 0
    );
  ...
  private calculateBatteryAggregation(solisData: any[]) {
    ...
    const chargedEnergyKwh = this.calculateTotalEnergy(
      solisData, 
      (item) => item.battery?.power > 0 ? item.battery.power : 0
    );

    const dischargedEnergyKwh = this.calculateTotalEnergy(
      solisData, 
      (item) => item.battery?.power < 0 ? Math.abs(item.battery.power) : 0
    );
  ...
  public async getAggregatedData(startDate: Date, endDate: Date): Promise<DailyAggregation[]> {
    return await this.dailyAggregationModel.find({
      date: {
        $gte: startDate,
        $lte: endDate
      }
